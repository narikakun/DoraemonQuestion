function initLightboxForImages(){document.querySelectorAll("a.lightbox-m1").forEach(el=>el.addEventListener("click",Lightbox.initialize))}$(function(){$.ajax({type:"GET",url:`/api/board/${classId}/board/${boardId}`,contentType:"application/json",dataType:"json"}).done(async function(data,textStatus,jqXHR){let boardData=data.data;let boardObj=boardData.data;$("#authorName").html(`投稿者: ${escapeHTML(boardData.author)}${boardData.teacher?` <span class="badge bg-secondary">教員</span>`:""}`);$("#postTitle").text(escapeHTML(boardObj.title));$("#createdAt").text(new Date(boardData.createdAt).toLocaleString("ja"));$("#content").html(boardObj.content?escapeHTML(boardObj.content).replace(/\r\n/g,"<br />"):"");let imgHtml="";for(const fileObj of boardObj.files){if(fileObj.pdf){imgHtml+=`
                    <div class="col">
                    ...
                    </div>`}}$("#imgList").html(imgHtml);initLightboxForImages("lightbox-m1");if(boardData.author==$.cookie("username")||$.cookie(`adminSession_${boardData.classId}`)){$("#replyBox").html(`<div class="card mb-3">
                    <div class="card-body">
                        <h4>コメント新規投稿</h4>
        ...
        <div class="col" id="comment_${comment._id}">
            ...
            `;for(const fileObj of comment.data.files){if(fileObj.pdf){replyBoxHtml+=`
            <div class="col-md-4">
